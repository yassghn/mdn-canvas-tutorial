...
const basicDrawingAndShapes = {
	...
	renderBezierAndQuadraticCurves(cvs, textEffect) {
		...
		const text = 'TODAYS NEWS @7: now that we are within the final 2.99 days before declaring the whole of civil structure in the country an armed force, what were the conditions issed (something between pissed and issued) for them to exit the war? my immediate release from my parents home with all parties responsible present (including illinois representatives). 400,000,000 million usd cash handed to me. the immediate exit of all civil structure, and anything or anyone even remotely related (yes, including private firms), from LIBERAL hold to the STATE with a period of rehabilitation. failure to meet any of these conditions will not be tolerated. many of these GodDAMMNED brain dead rats were wondering how we wiped out the fbi/cia/etc. and now they are finding out the hard way. 75 degrees of treason, enough to warrant summary executions with no trial. there is simply too much evidence.'
